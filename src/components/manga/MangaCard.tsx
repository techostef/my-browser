...
export default function MangaCard({ manga, onPress, onLongPress }: Props) {
  ...
  const lastReadDate = lastRead?.lastReadAt
    ? new Date(lastRead.lastReadAt).toLocaleDateString(undefined, { month: 'short', day: 'numeric' })
    : null;
  ...
  return (
    ...
    >
      {manga.coverImagePath ? (
        <Image source={{ uri: manga.coverImagePath }} style={styles.cover} resizeMode="cover" />
      ) : (
        <View style={[styles.cover, styles.coverPlaceholder, { backgroundColor: c.surfaceSecondary }]}>
      ...
      <View style={styles.info}>
        ...
        <View style={[styles.progressTrack, { backgroundColor: c.border }]}>
          <View
            style={[
              styles.progressFill,
              { width: `${progress * 100}%`, backgroundColor: progress === 1 ? '#34d399' : '#3b82f6' },
            ]}
          />
        </View>
        {downloading ? (

...
export default function ChapterItem({ chapter, onPress, onLongPress, onDelete, onRedownload, selected, selectionMode }: Props) {
  ...
  const isReadable = chapter.status === 'completed' || chapter.status === 'failed';
  const canInteract = selectionMode || isReadable;

  return (
    <View style={[styles.row, { backgroundColor: selected ? (c.border + '80') : c.surface, borderBottomColor: c.border }]}>

...
import { MangaTitle, MangaChapter } from '../types/manga';
...
type MangaAction =
  ...
  | { type: 'UPDATE_TITLE'; payload: { id: string; changes: Partial<MangaTitle> } }
  | { type: 'REMOVE_TITLE'; payload: { id: string } }
  | { type: 'UPDATE_CHAPTER'; payload: { mangaId: string; chapterId: string; changes: Partial<MangaChapter> } }
...
function reducer(state: MangaState, action: MangaAction): MangaState {
  switch (action.type) {
    ...
    case 'REMOVE_TITLE':
      return { ...state, titles: state.titles.filter(t => t.id !== action.payload.id) };
    case 'UPDATE_CHAPTER':
      return {
        ...state,
        titles: state.titles.map(t =>
          t.id === action.payload.mangaId
            ? {
                ...t,
                chapters: t.chapters.map(c =>
                  c.id === action.payload.chapterId ? { ...c, ...action.payload.changes } : c,
                ),
              }
            : t,
        ),
    ...
    case 'REMOVE_CHAPTER':
      return {
        ...
        titles: state.titles.map(t =>
          t.id === action.payload.mangaId
            ? { ...t, chapters: t.chapters.filter(c => c.id !== action.payload.chapterId) }
            : t,
        ),
      };
    default:
...
interface MangaContextValue {
  ...
  updateChapter: (mangaId: string, chapterId: string, changes: Partial<MangaChapter>) => void;
...
export function MangaProvider({ children }: { children: React.ReactNode }) {
  ...
  const updateTitle = useCallback((id: string, changes: Partial<MangaTitle>) => {
  ...
  const updateChapter = useCallback((mangaId: string, chapterId: string, changes: Partial<MangaChapter>) => {
    dispatch({ type: 'UPDATE_CHAPTER', payload: { mangaId, chapterId, changes } });
  }, []);

  const removeChapter = useCallback(async (mangaId: string, chapterId: string) => {

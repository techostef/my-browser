...
function reducer(state: Project[], action: Action): Project[] {
  switch (action.type) {
    case 'ADD':
      return [action.payload, ...state.filter(p => p.videoUri !== action.payload.videoUri)];
    case 'UPDATE':
...
interface ProjectContextValue {
  projects: Project[];
  addOrUpdateProject: (videoUri: string, videoName: string, duration: number) => Promise<Project>;
...
export function ProjectProvider({ children }: { children: React.ReactNode }) {
  ...
  const addOrUpdateProject = useCallback(
    async (videoUri: string, videoName: string, duration: number): Promise<Project> => {
      const existing = projects.find(p => p.videoUri === videoUri);
      if (existing) {
        const updated = { ...existing, videoName, duration, updatedAt: Date.now() };
        dispatch({ type: 'UPDATE', payload: { id: existing.id, videoName, duration, updatedAt: Date.now() } });
        return updated;
      }

      const id = Date.now().toString(36) + Math.random().toString(36).slice(2, 6);
      const now = Date.now();
      const project: Project = { id, videoUri, videoName, duration, createdAt: now, updatedAt: now };
      dispatch({ type: 'ADD', payload: project });

      // Generate thumbnail in background
      VideoThumbnails.getThumbnailAsync(videoUri, { time: 0 })
        .then(({ uri }) => dispatch({ type: 'UPDATE', payload: { id, thumbnailUri: uri } }))
        .catch(() => {});

...
function ExportScreen({ navigation, route }: Props) {
  ...
  const resolutions: Resolution[] = (() => {
    ...
    return [
      ...
      {
        label: "1080p",
        detail: `${t("fullHd")} · ${formatSize(estKbps(1080))}`,
        height: 1080,
      },
      {
        label: "720p",
        detail: `${t("hd")} · ${formatSize(estKbps(720))}`,
        height: 720,
      },
      {
        label: "480p",
        detail: `${t("smallerFile")} · ${formatSize(estKbps(480))}`,
        height: 480,
      },
    ];
  })();
  ...
  const renderSubtitlePngs = (
    ...
    return new Promise((resolve, reject) => {
      ...
      const html = buildSubtitleRenderHtml(
        segs,
        videoWidth,
        videoHeight,
        effectiveStyle,
  ...
  const handleWebViewMessage = (event: { nativeEvent: { data: string } }) => {
    ...
    if (msg.type === "png") {
      ...
      setStatus(
        t("renderingSubtitles", {
          index: String(msg.index + 1),
      ...
      if (pngPhaseRef.current) {
        ...
        setProgress(
          start + ((msg.index + 1) / Math.max(1, total)) * (end - start),
        );
      }
      ...
      pngRejectRef.current?.(
        new Error(`Subtitle render failed: ${msg.message}`),
      );
      finishWebView();
    }
  };
  ...
  const runExport = async (targetHeight: number | null) => {
    try {
      ...
      const outputDurationSec = keptRanges.reduce(
      ...
      if (hasSubs) {
        ...
        const pngResults = await renderSubtitlePngs(
          subtitleSegments!,
          renderW,
          renderH,
        );
        ...
        if (!blank)
          throw new Error("Subtitle renderer did not produce a blank PNG");

...
function MangaChapterListScreen() {
  ...
  const { getTitle, removeTitle, removeChapter } = useManga();
  ...
  const lastReadChapter = useMemo(() => {
    ...
    const readable = manga.chapters.filter(
      (ch) => ch.status === "completed" && ch.lastReadAt,
    );
    if (!readable.length) return null;
    return readable.reduce((a, b) =>
  ...
  const handleDeleteChapter = useCallback(
    (chapterId: string, chapterTitle: string) => {
      ...
      Alert.alert(
        `Delete "${chapterTitle}"?`,
        "This will delete the downloaded images.",
        [
          { text: "Cancel", style: "cancel" },
          {
            text: "Delete",
            style: "destructive",
            onPress: () => removeChapter(manga.id, chapterId),
          },
        ],
      );
    },
  ...
  const handleRedownloadChapter = useCallback(
    (chapterId: string, chapterNumber: string) => {
      ...
      Alert.alert(
        "Redownload chapter?",
        `Re-download Ch. ${chapterNumber}? Existing files will be replaced.`,
        [
          { text: "Cancel", style: "cancel" },
          {
            text: "Redownload",
  ...
  const handleMenuPress = () => {
    ...
    if (failedChapters.length > 0) {
      options.push({
        text: `Retry All Failed (${failedChapters.length})`,
  ...
  const handleChapterPress = (chapterId: string) => {
    ...
    if (chapter?.status === "completed") {
      ...
      Alert.alert(
        "Retry download?",
        `Re-download Ch. ${chapter.chapterNumber}?`,
        [
          { text: "Cancel", style: "cancel" },
          {
            text: "Retry",
  ...
  const handleBulkRedownload = () => {
    ...
    if (completedSelected.length === 0) {
      Alert.alert(
        ...
        "Only completed chapters can be redownloaded.",
      );
      return;
    }
    Alert.alert(
  ...
  const handleBulkDelete = () => {
    ...
    if (deletable.length === 0) {
      Alert.alert(
        "Nothing to delete",
        "Only completed or failed chapters can be deleted.",
      );
      return;
    }
    Alert.alert(
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: c.background }]}>
      <View style={[styles.header, { borderBottomColor: c.border }]}>
        {selectionMode ? (
          ...
          <>
            ...
            >
              <Text style={[styles.backText, { color: "#3b82f6" }]}>
                ← Back
              </Text>
            </TouchableOpacity>
            <Text
      ...
      {!selectionMode &&
        (failedChapters.length > 0 || queuedChapters.length > 0) && (
          <View
            style={[
              styles.retryAllBar,
            ...
            {failedChapters.length > 0 && (
              ...
              >
                <Text style={styles.retryAllBtnText}>
                  ↻ Retry All Failed ({failedChapters.length})
                </Text>
              </TouchableOpacity>
            )}
          </View>
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <ChapterItem
              ...
              onDelete={() =>
                handleDeleteChapter(item.id, `Ch. ${item.chapterNumber}`)
              }
              onRedownload={() =>
                handleRedownloadChapter(item.id, item.chapterNumber)
              }
      ...
      {selectionMode && (
        ...
        >
          <TouchableOpacity
            style={[styles.actionBtn, styles.redownloadActionBtn]}
            onPress={handleBulkRedownload}
          >
      ...
      {!selectionMode && lastReadChapter && (
        ...
        >
          <View style={styles.continueInfo}>
            <Text style={[styles.continueLabel, { color: c.textSecondary }]}>
              Continue Reading
            </Text>
            <Text
              style={[styles.continueChapter, { color: c.text }]}
              numberOfLines={1}
            >
            ...
            <Text style={[styles.continueMeta, { color: c.textSecondary }]}>
              ...
              {lastReadChapter.imageCount > 0
                ? ` of ${lastReadChapter.imageCount}`
                : ""}
